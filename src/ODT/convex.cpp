/*
 * modified C++ cide from code from the article
 * "Testing the Convexity of a Polygon"
 * by Peter Schorn and Frederick Fisher,
 *	(schorn@inf.ethz.ch, fred@kpc.com)
 * in "Graphics Gems IV", Academic Press, 1994

 // refer to http://erich.realtimerendering.com/ptinpoly/#ref9
 */
...
int GetPoint(FILE *f, Point2d *p)		/* Read p's x- and y-coordinate from f	*/
 /* and return true, iff successful.	*/
{
    return !feof(f) && (2 == fscanf(f, "%lf%lf", &(p->x), &(p->y)));
}

int GetDifferentPoint(FILE *f, Point2d previous, Point2d *next)
...
PolygonClass ClassifyPolygon(FILE *f)
{
