...
#define intervalmax_imgdual 2/27.00    // time interval of img msg from different camera


using namespace CVD;
using namespace std;

class DualMonoSLAMnodelet : public BasePTAMNode{
public:
    DualMonoSLAMnodelet() : BasePTAMNode(){}
...
//        tracker_->load_reflandingpad(ref_img_path);
        ...
        image_sub_ = it.subscribe(image_, 1, boost::bind(&DualMonoSLAMnodelet::imageCallback, this, _1));
        image_sub_sec = it.subscribe(image_sec, 1, boost::bind(&DualMonoSLAMnodelet::imageSecondCallback, this, _1));
        image4ini_sub_ = it.subscribe("/simple_landing/image_ini_ptam", 1, boost::bind(&DualMonoSLAMnodelet::image_ini_Callback, this, _1));
        sub_attitude_ = nh_.subscribe<pximu::AttitudeData>("/attitude", 1, boost::bind(&DualMonoSLAMnodelet::attitudeCallback, this, _1));
        sub_pose_ekf_ = nh_.subscribe<geometry_msgs::PoseWithCovarianceStamped>("/ssf_core/pose_predicted_ekf", 1, boost::bind(&DualMonoSLAMnodelet::pose_predicted_ekfCallback, this, _1));
        sub_pose_simplenode_ = nh_.subscribe<geometry_msgs::TransformStamped>("/simple_landing/vicon_pose_ptam", 1, boost::bind(&DualMonoSLAMnodelet::viconCallback, this, _1));
...
//            cout << pose->pose.covariance[6] << ", " << pose->pose.covariance[7] << ", " << pose->pose.covariance[8] << endl;
    ...
    void publish_inilandingpadpose(ros::Time stamp)
    {
        ...
        geometry_msgs::TransformStampedPtr padpose;
    ...
    // The image callback for the second camera.
    // Pass the received image to the master callback function
    // Tricky is how to retrieve the synchronised dual images
    void imageSecondCallback(const sensor_msgs::ImageConstPtr& img_msg){
        secondcamlock.lock();
        img_secmsg = img_msg;
        secondimgcame = true;
        secondcamlock.unlock();
//        ROS_INFO("SECOND IMG TIMESTAMP: %f", img_msg->header.stamp.toSec());
    }
...
private:
    ...
    void convertImage(const sensor_msgs::ImageConstPtr& img_msg, int camnum) {
...
PLUGINLIB_DECLARE_CLASS(dmslam, DualMonoSLAMnodelet,
                        DualMonoSLAMnodelet, nodelet::Nodelet);

int main(int argc, char **argv)
{
    ros::init(argc, argv, "dualmono_nodelet");

    DualMonoSLAMnodelet nodelet;
