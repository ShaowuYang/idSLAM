...
backend::backend(ros::NodeHandle &nh, ros::NodeHandle &nh_pri, SLAMSystem &ss) :
    nh_(nh),
    nh_private_(nh_pri),
    slam_(ss)
{
...
void backend::addKeyframe(const int kf_id)
{
...
void backend::addEdges(const std::vector<ptam::Edge> msg)
{
...
std::vector<boost::shared_ptr<ptam::Edge> > backend::messageToEdges(const std::vector<ptam::Edge>& msg)
{
