...
public:
    ...
    std::vector<boost::shared_ptr<Keyframe> > wlKeyFrames;  //kfs, the waiting list to be added by the backend
    std::vector<boost::shared_ptr<Edge> > wlEdges;  //kfs, the waiting list to be added by the backend
    const std::vector<boost::shared_ptr<Keyframe> >& keyframes() { return keyframes_; }
    std::vector<boost::shared_ptr<Keyframe> >& wlkf() { return wlKeyFrames; }
    ...
    std::vector<boost::shared_ptr<Edge> > ptam_edges_; // ptam edges, indexed by kf A index idA
    std::vector<boost::shared_ptr<Edge> > ptam_edges_local_;// ptam edges in local BA come with the new kf at the same time
