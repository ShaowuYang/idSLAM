...
ATANCamera::ATANCamera(string sName)
{
  ...
  SetImageSize(CVD::ImageRef(640, 480));
}
...
void ATANCamera::RefreshParams() 
{
  ...
  mvFocal[0] = GetImageSize()[0] * (*mgvvCameraParams)[0];
  mvFocal[1] = GetImageSize()[1] * (*mgvvCameraParams)[1];
  mvCenter[0] = GetImageSize()[0] * (*mgvvCameraParams)[2] - 0.5;
  mvCenter[1] = GetImageSize()[1] * (*mgvvCameraParams)[3] - 0.5;
  ...
  {
    Vector<2> v2Center = UnProject(GetImageSize() / 2);
    Vector<2> v2RootTwoAway = UnProject(GetImageSize() / 2 + vec(ImageRef(1,1)));
...
Vector<2> ATANCamera::Project_ud(const Vector<2>& vCam){
  mvLastCam = vCam;
  mdLastR = sqrt(vCam * vCam);
  mbInvalid = (mdLastR > mdMaxR);
  mdLastFactor = rtrans_factor(mdLastR);
...
Vector<2> ATANCamera::UnProject_ud(const Vector<2>& v2Im)
{
  mvLastIm = v2Im;
  mvLastDistCam[0] = (mvLastIm[0] - mvCenter[0]) * mvInvFocal[0];
  mvLastDistCam[1] = (mvLastIm[1] - mvCenter[1]) * mvInvFocal[1];
...
Vector<2> ATANCamera::UnProjectSafe(const Vector<2>& v2Im) const {
