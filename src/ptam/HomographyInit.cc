...
bool HomographyInit::TransIniFromCircle(SE3<>se3First, SE3<> se3Second, double disdifmin, double &scaleSecondFromFirst){
   bool isgood = false;
   Vector<3> transSecondFromFirst;
//   transSecondFromFirst = se3Second.get_rotation().inverse() * (se3First.get_translation()-se3Second.get_translation());
   transSecondFromFirst = se3Second.get_translation() - se3Second.get_rotation() * se3First.get_rotation().inverse() * se3First.get_translation();

//   double disdif = sqrt(se3SecondFromFirst.get_translation()[0]*se3SecondFromFirst.get_translation()[0] +
//                        se3SecondFromFirst.get_translation()[1]*se3SecondFromFirst.get_translation()[1]);
   double disdif = sqrt(transSecondFromFirst*transSecondFromFirst);

   if (disdif > disdifmin)
       isgood = true;

   scaleSecondFromFirst = disdif;
