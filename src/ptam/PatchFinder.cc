...
void PatchFinder::MakeTemplateCoarse(MapPoint &p,
				     ...
				     Matrix<2> &m2CamDerivs)
{
  CalcSearchLevelAndWarpMatrix(p, se3CFromW, m2CamDerivs)>=0;
...
void PatchFinder::MakeTemplateCoarseCont(MapPoint &p)
{
  ...
  if(bNeedToRefreshTemplate && !p.pPatchSourceKF.expired())
    {
      ...
      nOutside = CVD::transform(p.pPatchSourceKF.lock()->aLevels[p.nSourceLevel].im, 
				mimTemplate, 
				m2,
				vec(p.irCenter),
				vec(mirCenter)); 
...
void PatchFinder::MakeTemplateCoarseNoWarp(MapPoint &p)
{
  ...
  if(kf.get() != NULL)
	  MakeTemplateCoarseNoWarp(*kf, p.nSourceLevel,  p.irCenter);
}
...
bool PatchFinder::FindPatchCoarse(ImageRef irPos, KeyFrame &kf, unsigned int nRange)
{
    ...
    vector<double>::iterator i_depth;

    assert(L.vCorners.size() == L.vCornersDepth.size());

    i = L.vCorners.begin() + L.vCornerRowLUT[nTop];
    i_depth = L.vCornersDepth.begin() + L.vCornerRowLUT[nTop];
    ...
    double dBestDepth = 0.0;

    for(; i < i_end; i++, i_depth++) {          // For each corner ...
    ...
    if(nBestSSD < mnMaxSSD)      // Found a valid match?
    {
        ...
        mdCoarseDepth = dBestDepth;
        if (isnan(mdCoarseDepth))
            mdCoarseDepth = 0.0;
        mbFound = true;
    }
    else
